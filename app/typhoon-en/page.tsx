...
function MetricCard({
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", alignItems: "center", padding: "10px 8px", background: "var(--tp-card-bg, rgba(255,255,255,0.03))", borderRadius: 8, border: "1px solid rgba(255,255,255,0.05)" }}>
      ...
      <div style={{ fontSize: 10, color: "var(--tp-text-muted, #6B7B95)", marginBottom: 2 }}>{label}</div>
      <div style={{ fontSize: 18, fontWeight: 700, fontFamily: "var(--font-jetbrains-mono), monospace", color: "var(--tp-text, #e0e6f0)", lineHeight: 1.2 }}>{value}</div>
  ...
  return (
    <div>
      <div style={{ display: "flex", alignItems: "center", gap: 6, marginBottom: 12 }}>
        <TrendingDown size={14} style={{ color: "var(--tp-accent, #3BD5FF)" }} />
        <span style={{ fontSize: 12, fontWeight: 600, textTransform: "uppercase", letterSpacing: "0.08em", color: "var(--tp-text-secondary, #8D9AAF)" }}>Intensity Trend</span>
      </div>
        ...
        {timeline.map((t, i) => {
          ...
          return (
            <button key={i} onClick={() => onPointSelect({ lat: t.lat, lng: t.lng, time: t.time, pressure: t.pressure, windSpeed: t.windSpeed, isForecast: t.isForecast, hoursAhead: t.hoursAhead })}
              style={{ display: "flex", alignItems: "center", gap: 8, padding: "6px 8px", borderRadius: 6, border: sel ? `1px solid ${catInfo.color}40` : "1px solid transparent", background: sel ? `${catInfo.color}10` : "transparent", cursor: "pointer", width: "100%", textAlign: "left", fontFamily: "inherit", color: "var(--tp-text, #e0e6f0)", transition: "background 0.15s" }}>
              ...
              <div style={{ flex: 1, display: "flex", flexDirection: "column", gap: 3 }}>
                <div style={{ display: "flex", alignItems: "center", gap: 4 }}>
                  <span style={{ fontSize: 9, color: "var(--tp-text-muted, #6B7B95)", width: 40, flexShrink: 0 }}>{t.pressure} hPa</span>
                ...
                <div style={{ display: "flex", alignItems: "center", gap: 4 }}>
                  <span style={{ fontSize: 9, color: "var(--tp-text-muted, #6B7B95)", width: 40, flexShrink: 0 }}>{t.windSpeed} kt</span>
              ...
              <div style={{ fontSize: 9, color: "var(--tp-text-muted, #6B7B95)", flexShrink: 0, width: 55, textAlign: "right" }}>{new Date(t.time).toLocaleDateString("en", { month: "short", day: "numeric", hour: "2-digit" })}</div>
            </button>
          );
        })}
      </div>
...
export default function TyphoonEnPage() {
  ...
  if (loading && !data) {
    return (
      <div style={{ minHeight: "100dvh", background: colors.bg, display: "flex", alignItems: "center", justifyContent: "center", flexDirection: "column", gap: 16 }}>
        ...
        <p style={{ color: "var(--tp-text-secondary, #8D9AAF)", fontSize: 14 }}>Fetching typhoon data...</p>
  ...
  return (
    <div style={{ minHeight: "100dvh", background: colors.bg, color: "var(--tp-text, #e0e6f0)", "--tp-bg-panel": colors.bgPanel, "--tp-border": colors.border, "--tp-text": colors.text, "--tp-text-muted": colors.textMuted, fontFamily: "system-ui, -apple-system, sans-serif" } as React.CSSProperties}>
      {/* HEADER */}
      <header style={{ position: "sticky", top: 0, zIndex: 50, background: colors.headerBg, backdropFilter: "blur(12px)", borderBottom: `1px solid ${colors.borderAccent}`, padding: "10px 16px", display: "flex", alignItems: "center", justifyContent: "space-between", flexWrap: "wrap", gap: 8 }}>
        <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
          <AlertTriangle size={18} style={{ color: catInfo?.color ?? "#E53E3E" }} />
          <span style={{ fontSize: 11, letterSpacing: "0.1em", color: "var(--tp-text-secondary, #8D9AAF)" }}>TYPHOON MONITOR</span>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 12, fontSize: 12 }}>
          ...
          <span style={{ color: "var(--tp-text-muted, #6B7B95)" }}><Clock size={11} style={{ display: "inline", marginRight: 3 }} />{storm?.issueTime ? new Date(storm.issueTime).toLocaleString("en", { month: "short", day: "numeric", hour: "2-digit", minute: "2-digit" }) : "---"}</span>
          <button onClick={() => fetchData(true)} disabled={loading}
            style={{ display: "flex", alignItems: "center", gap: 4, padding: "4px 10px", borderRadius: 6, border: "1px solid rgba(59, 213, 255, 0.3)", background: "rgba(59, 213, 255, 0.08)", color: "var(--tp-accent, #3BD5FF)", cursor: "pointer", fontSize: 11, fontWeight: 600, opacity: loading ? 0.5 : 1 }}
      ...
      <main style={{ display: "flex", flexDirection: "column" }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12, padding: "12px 16px", maxWidth: 1200, margin: "0 auto", width: "100%", boxSizing: "border-box" }} className="typhoon-en-panels">
          ...
          <div className="panel-card">
            <div style={{ display: "flex", alignItems: "center", gap: 6, marginBottom: 12 }}>
              <Shield size={14} style={{ color: "var(--tp-accent, #3BD5FF)" }} />
              <span style={{ fontSize: 12, fontWeight: 600, textTransform: "uppercase", letterSpacing: "0.08em", color: "var(--tp-text-secondary, #8D9AAF)" }}>Official Info &amp; Safety</span>
            </div>
            <div style={{ marginBottom: 14 }}>
              <div style={{ fontSize: 10, color: "var(--tp-text-muted, #6B7B95)", marginBottom: 8, textTransform: "uppercase", letterSpacing: "0.05em" }}>Official Warning Portals</div>
              <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
                {OFFICIAL_SOURCES.map((src) => (
                  <a key={src.url} href={src.url} target="_blank" rel="noopener noreferrer"
                    style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "8px 10px", borderRadius: 6, background: "rgba(59, 213, 255, 0.04)", border: "1px solid var(--tp-border, rgba(59,213,255,0.12))", textDecoration: "none", color: "var(--tp-text, #e0e6f0)", transition: "background 0.15s" }}>
                    <div>
                      <div style={{ fontSize: 12, fontWeight: 500 }}>{src.name}</div>
                      <div style={{ fontSize: 10, color: "var(--tp-text-muted, #6B7B95)", marginTop: 1 }}>{src.desc}</div>
                    </div>
                    <ExternalLink size={12} style={{ color: "var(--tp-text-muted, #6B7B95)", flexShrink: 0 }} />
                  </a>
                ))}
              </div>
            </div>
            <div>
              ...
              <ul style={{ margin: 0, padding: "0 0 0 16px", fontSize: 11, color: "var(--tp-text-secondary, #8D9AAF)", lineHeight: 1.8 }}>
      ...
      {/* FOOTER */}
      <footer style={{ textAlign: "center", padding: "12px 16px", fontSize: 10, color: "var(--tp-text-muted, #6B7B95)", borderTop: `1px solid ${colors.border}`, background: colors.footerBg }}>
        ...
        <p style={{ margin: "4px 0 0", opacity: 0.6 }}>Copyright &copy; 2026 John Zhou | <a href="/" style={{ color: "var(--tp-accent, #3BD5FF)", textDecoration: "none" }}>Planet Pulse</a></p>
      </footer>

      <style>{`
        .panel-card { background: colors.bgPanel; backdrop-filter: blur(12px); -webkit-backdrop-filter: blur(12px); border: 1px solid rgba(59, 213, 255, 0.12); border-radius: 12px; padding: 14px; overflow: hidden; }
